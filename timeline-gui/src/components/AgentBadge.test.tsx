...
describe('AgentBadge', () => {
  ...
  describe('Avatar Generation', () => {
    it('should render avatar with initials from display name', () => {
      ...
      // Find the AvatarFallback element that contains the initials
      const fallback = avatar.querySelector('[data-slot="avatar-fallback"]');
      expect(fallback).toHaveTextContent('TA');
    });

    it('should generate initials from multi-word display name', () => {
      render(<AgentBadge agentName="claude-assistant" displayName="Claude AI Assistant Helper" />);

      const avatar = screen.getByTitle('Claude AI Assistant Helper (@claude-assistant)');
      const fallback = avatar.querySelector('[data-slot="avatar-fallback"]');
      expect(fallback).toHaveTextContent('CA'); // First two initials
    });

    it('should generate initials from single word display name', () => {
      render(<AgentBadge agentName="bot" displayName="Bot" />);
    ...
    it('should generate consistent colors for same agent name', () => {
      ...
      const firstFallback = firstAvatar.querySelector('[data-slot="avatar-fallback"]');
      const firstColorClass = Array.from(firstFallback!.classList).find(cls =>
        cls.startsWith('bg-')
      );
      ...
      const secondFallback = secondAvatar.querySelector('[data-slot="avatar-fallback"]');
      const secondColorClass = Array.from(secondFallback!.classList).find(cls =>
        cls.startsWith('bg-')
      );
    ...
    it('should generate different colors for different agent names', () => {
      ...
      rerender(<AgentBadge agentName="different-agent" displayName="Different Agent" />);
  ...
  describe('Agent Information Display', () => {
    ...
    it('should not display handle when display name equals agent name', () => {
      render(<AgentBadge agentName="test-agent" displayName="test-agent" />);

      expect(screen.queryByText('@test-agent')).not.toBeInTheDocument();
    });

    it('should convert spaces to underscores in handle', () => {
      render(<AgentBadge agentName="test agent with spaces" displayName="Test Agent" />);
  ...
  describe('Accessibility', () => {
    ...
    it('should have proper semantic structure', () => {
      ...
      // Should have avatar element with shadcn/ui structure
      const avatar = screen.getByTitle('Test Agent (@test-agent)');
